...
- **后端**：Spring Boot 3.4.7 + MyBatis + MySQL + JWT + Redis
- **前端**：Vue 3 + Vite + Element Plus + Pinia
- **认证**：JWT Token认证 + Redis黑名单机制
- **日志**：通过AOP实现操作日志记录（密码自动脱敏）
- **缓存**：集成Redis缓存优化系统性能
- **其他**：集成Swagger API文档、Flyway数据库迁移、支持跨域访问
...
| 数据库 | MySQL / Redis |
| 安全认证 | JWT + BCrypt / Spring Security / Redis黑名单 |
| 前端 | Vue 3 / Vite / Element Plus / Pinia |
| 工具 | SpringDoc（API文档）、AOP（日志记录）、Lombok、Hutool、Flyway |
...
### 1. 用户管理模块
- **用户注册与登录**：支持普通用户注册和登录功能（带分布式锁防重复注册）
- **用户信息维护**：用户可以查看和修改自己的信息（包括头像上传）
- **密码修改**：支持用户修改登录密码（BCrypt加密，改密后Token立即失效）
- **分页查询**：支持用户信息的分页展示
- **RBAC权限**：基于角色的访问控制，支持多角色体系

### 2. 管理员管理模块
- **多角色管理员**：支持超级管理员、部门管理员、社团负责人三种管理员角色
- **管理员登录**：支持管理员登录功能
- **管理员信息维护**：管理员可以管理自己的信息
- **密码修改**：支持管理员修改登录密码（BCrypt加密，改密后Token立即失效）
- **用户管理**：超级管理员可以管理所有用户和管理员信息
- **通知管理**：管理员可以向用户发送通知
...
### 5. 安全认证模块
- **JWT Token**：基于JWT的无状态认证机制（固定密钥 + Redis黑名单）
- **Token验证**：自动验证Token的有效性，支持Token立即失效
- **BCrypt加密**：使用BCrypt算法加密存储密码（自动加盐，防彩虹表攻击）
- **角色权限**：基于RBAC的多角色权限控制（SUPER_ADMIN/DEPT_ADMIN/CLUB_LEADER/USER）
- **策略模式**：使用策略模式处理不同角色的登录和密码更新逻辑
- **审计日志脱敏**：自动脱敏敏感信息（如密码字段）
...
### 8. 缓存优化模块
- **Redis集成**：集成Redis作为分布式缓存
- **数据缓存**：对用户信息、菜单数据等高频访问数据进行缓存（30分钟过期）
- **Token黑名单**：登出或改密码后将旧Token加入黑名单，立即失效
- **分布式锁**：使用Redis实现分布式锁，防止重复注册
- **缓存更新**：数据更新时自动清除相关缓存
...
- `users`：统一用户表（包含普通用户和管理员，通过role字段区分）
- `user_role`：用户角色关联表（RBAC权限系统）
- `role`：角色定义表（SUPER_ADMIN/DEPT_ADMIN/CLUB_LEADER/USER）
- `menu`：菜单表（菜单名称、路径、父菜单、权限等）
- `audit_log`：操作日志表（用户名、操作、资源、IP地址、详情等，密码自动脱敏）
- `notification`：通知表（通知内容、接收者、状态等）
- `flyway_schema_history`：Flyway数据库版本管理历史表

> SQL脚本位于：`springboot/src/main/resources/db/migration/V1__init_schema.sql`
...
2. 修改对应环境的配置文件（application-dev.yml / application-test.yml / application-prod.yml）中的数据库连接信息：
   ```yaml
   spring:
     datasource:
...
### 1. 安全性
- JWT Token认证 + Redis黑名单机制，保障接口安全
- BCrypt密码加密存储（自动加盐，防彩虹表攻击）
- 自定义JWT认证过滤器，验证请求合法性
- RBAC多角色权限控制，防止越权访问
- 审计日志敏感信息自动脱敏（如密码字段）
- 文件上传需认证，文件下载可公开访问
...
项目集成了SpringDoc OpenAPI文档，启动后端服务后可访问 http://localhost:9991/swagger-ui.html 查看完整的API文档。

### 主要接口分类：
- `/user/**`：用户相关接口（增删改查、分页查询）
- `/admin/**`：管理员相关接口（增删改查、分页查询）
- `/menu/**`：菜单相关接口（根据角色获取菜单树）
- `/files/**`：文件相关接口（上传需认证、下载可公开）
- `/notification/**`：通知相关接口（发送、获取、标记已读、删除）
- `/api/audit/**`：审计日志相关接口（查询最近操作日志）
...
近期项目进行了多项重要更新，主要包括以下几个方面：

### 功能增强
- **Redis集成**：添加了Redis缓存功能以优化系统性能，包括用户信息缓存、Token黑名单、分布式锁
- **角色升级**：扩展为四角色体系（SUPER_ADMIN/DEPT_ADMIN/CLUB_LEADER/USER），支持RBAC权限控制
- **密码安全**：从MD5升级为BCrypt加密算法，自动加盐防彩虹表攻击
- **动态菜单**：实现了动态菜单系统，可根据用户角色加载相应菜单
- **文件管理**：修复了文件下载接口返回类型和响应头设置问题
- **主题切换**：完整实现日间/夜间模式切换功能
- **注册功能**：开放用户注册，带分布式锁防止重复注册

### 架构优化
- **配置优化**：分离了多环境配置文件并添加环境激活机制（开发、测试、生产环境）
- **Mapper重构**：将注解配置迁移到XML配置文件，提升了可维护性
- **管理员重构**：重构了管理员实体和数据访问层，统一使用users表
- **前端重构**：重构了前端界面和路由结构，引入Pinia状态管理
- **白名单统一**：统一JwtAuthenticationFilter和SecurityConfig的白名单策略

### 性能与安全
- **JWT安全**：采用固定密钥 + Redis黑名单机制，改密码/登出后Token立即失效
- **身份验证**：添加了token验证功能，增强了系统安全性
- **路由保护**：添加路由守卫防止已登录用户访问登录页面，并验证Token有效性
- **审计日志脱敏**：自动脱敏敏感信息（如密码字段）
- **文件权限**：文件上传需认证，文件下载可公开访问

### 用户体验
- **头像显示**：修复了登录后左上角头像显示问题
- **修改密码**：修复了原密码显示加密字符串的问题，改密后自动清理Token
- **界面优化**：隐藏了脚手架管理系统标题，优化了界面显示
- **登录优化**：移除了扫码登录功能并简化了登录界面

### 构建与配置
- **Flyway集成**：添加Flyway数据库版本管理，自动执行迁移脚本
- **SpringDoc**：从Swagger迁移到SpringDoc OpenAPI
- **构建优化**：配置了资源过滤和编译编码
- **文件配置**：配置了文件上传基础URL和路径
- **Git配置**：添加了dump.rdb到.gitignore以排除临时文件
